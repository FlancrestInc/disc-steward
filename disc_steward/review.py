...
def seed_automatic_review(
    ...
    automatic_labels: dict[str, AutomaticLabel] | None = None,
) -> dict[str, list[str]]:
    ...
    for scanned in scanned_files:
        ...
        automatic_label = (automatic_labels or {}).get(scanned.path)
        fields: list[str] = []
        role = automatic_label.role if automatic_label else _automatic_role(scanned, classification)
        content_type = (
            automatic_label.content_type
            if automatic_label and automatic_label.content_type
            else "movie"
            if role == "main_feature"
            else "show"
            if role == "episode"
            else "extra"
            if role
        ...
        display_name = automatic_label.display_name if automatic_label else _automatic_display_name(scanned, role)
        ...
        if automatic_label:
            set_if_blank("extra_type", automatic_label.extra_type)
            set_if_blank("season_number", automatic_label.season_number)
            set_if_blank("episode_number", automatic_label.episode_number)
        set_if_blank("encoding_profile", config.preferred_video_profile)
        set_if_blank("subtitle_policy", subtitle_policy)
        if automatic_label:
            note = f"Automatic bonus labeling ({automatic_label.confidence:.0%}): {automatic_label.evidence}"
            if decision.notes:
                if note not in decision.notes:
                    decision.notes = f"{decision.notes}; {note}"
                    fields.append("notes")
            else:
                decision.notes = note
                fields.append("notes")
        elif classification.reasons and not decision.notes:
...
def clear_automatic_review_labels(db, job_id: int) -> int:
    """Remove labels previously generated by the automatic reviewer only."""
    cleared = 0
    for decision in db.list_file_reviews(job_id):
        if not decision.notes or not decision.notes.startswith("Automatic bonus labeling"):
            continue
        decision.role = ""
        decision.content_type = "unknown"
        decision.final_display_name = None
        decision.extra_type = None
        decision.season_number = None
        decision.episode_number = None
        decision.notes = None
        db.save_file_review(decision)
        cleared += 1
    if cleared:
        db.audit("automatic_review_cleared", f"Cleared {cleared} prior automatic labels before Hermes review", job_id)
    return cleared
